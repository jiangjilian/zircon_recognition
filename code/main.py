...
def my_train(methods, IS_TRAIN = False):
    for method in methods:
        ...
        if_low_P_S_type = (processed_data["Set"] == "Testing set") & (processed_data["P_raw"] <= 20) & (
                    processed_data["Zircon"] == "S-type zircon")
        ...
        JH_S_zircon_proportion = sum(
            (processed_data['Zircon'] == "JH zircon") & (processed_data[method + ' Label'] == 1)) / sum(
            processed_data['Zircon'] == "JH zircon")
        JH_Hadean_S_proportion = sum(
            (processed_data['Zircon'] == "JH zircon") & (processed_data[method + ' Label'] == 1) & (
                    zircons_data['Age(Ma)'] >= 4000)) / sum(
            (processed_data['Zircon'] == "JH zircon") & (zircons_data['Age(Ma)'] >= 4000))
...
if __name__ == '__main__':
    ...
    zircons_data.loc[zircons_data["Zircon"] == "TTG zircon", "Label"] = 0
    ...
    # 2. Train or evaluate machine learning models
    my_train(methods, IS_TRAIN=False)


    # 3. Learning curve and hyperplane function for TSVM
    ## Load model: TSVM
    method = "tsvm"
    ...
    # 4. Update prediction results in database

    zircons_data["Set"] = processed_data['Set']
    for set_type, detrital_label_base in [("Prediction set", "detrital"), ("Training set", ""), ("Testing set", "")]:
        zircons_data.loc[(zircons_data["Set"] == set_type) & (zircons_data[method +' Label'] == 0), method + " model"] = f"non-S-type {detrital_label_base} zircon"
    ...
    train_test_data[f'Performance of {method} model'] = np.where(train_test_data['Label'] == train_test_data[method + ' Label'], 'correct', 'wrong')
    ...
    # 5. Predict global detrital zircons and JH zircon, compute the bootstrap of S-type zircon
    ...
    GSSB_data = pd.read_excel(data_path + "GSSB_all_zircon.xlsx")
    GSSB_ratio_seq = predict_and_save_time_series(x_train,
                                                     pred_data=GSSB_data,
                                                     method=method,
                                                     output_filename="Bootstrap_means_GSSB_zircon_" + method + ".csv")